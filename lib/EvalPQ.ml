...
let rec eval_stmt (p: _ prog) (n: meta node) (s: stmt) (m: metric) = 
  ...
  | ChildrenCall(proc_name) -> 
    List.iter n.children (fun new_node -> eval_stmts p new_node (stmts_of_processed_proc p proc_name) m)
...
let eval (p: _ prog) (n: meta node) (m: metric) = List.iter p.order ~f:(fun pass_name -> eval_stmts p n (stmts_of_processed_proc p pass_name) m)
...
let add_children (p: _ prog) (x: meta node) (y: meta node) (n: int) (m: metric): unit =
  ...
  Hashtbl.iter p.procs ~f:(fun (ProcessedProc(proc, _)) -> 
    ...
    let time: TotalOrder.t = 
      match down, up with
      | (Some d, _) -> Hashtbl.find_exn prev.m.time_table d
      | _ -> panic "todo"
    in fix_init (ref time) y down up)
...
let recalculate (p: _ prog) (_: meta node) (m: metric) = 
  recalculate_aux p m;
