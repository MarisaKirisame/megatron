...
let queue_push x y z : unit =
  if PriorityQueue.add queue (x, y, z) then ()
...
let prop_modified (p : _ prog) (n : meta node) (prop_name : string) (m : metric)
    : unit =
      ...
      Hashtbl.iter p.bbs ~f:(fun (BasicBlock (bb_name, stmts)) ->
          ...
          let dirty read =
            ...
            | ReadHasPath First | ReadHasPath Last ->
                if List.is_empty x.children then bb_dirtied x bb_name m else ()
            | ReadProp (First, _) ->
...
let rec fix_init (current_time : TotalOrder.t ref) (x : meta node)
    (down : string option) (up : string option) : unit =
  (match down with
  | Some d ->
      current_time := TotalOrder.add_next !current_time;
  ...
  | None -> ());
  List.iter x.children ~f:(fun children ->
      fix_init current_time children down up);
  match up with
  | Some u ->
      current_time := TotalOrder.add_next !current_time;
...
let add_children (p : _ prog) (x : meta node) (y : meta node) (n : int)
  ...
  Hashtbl.iter p.bbs ~f:(fun (BasicBlock (bb_name, stmts)) ->
      ...
      let dirty read =
        ...
        | ReadHasPath Parent | ReadProp (Parent, _) -> ()
        | ReadHasPath First | ReadHasPath Last ->
            if phys_equal (List.length x.children) 1 then bb_dirtied x bb_name m
            else ()
        | ReadProp (First, _) ->
            if List.is_empty lhs then bb_dirtied x bb_name m else ()
        | ReadProp (Last, _) ->
