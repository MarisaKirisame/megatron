...
module Make (O : Set.OrderedType): (S with type data = O.t) = struct
  ...
  let rec add_node queue x = match !queue with
  ...
  let add queue x = let added = add_node queue.node x in (if added then queue.size <- queue.size + 1 else ()); added

  let rec pop_node queue = match !queue with
  ...
  let pop queue = let v = pop_node queue.node in queue.size <- queue.size - 1; v

  let rec peek_node queue = match !queue with
      | Node ( value, ({ contents=Node _ } as left), _ ) ->
          peek_node left
      | Node ( value, { contents=Null }, right ) ->
          value
  ...
  let rec remove_node queue x = match !queue with
      | Node ( value, left, right ) ->
          let dir = O.compare x value in
          if dir == 0 then begin
  ...
  let remove queue x = let removed = remove_node queue.node x in (if removed then queue.size <- queue.size - 1 else ()); removed
end