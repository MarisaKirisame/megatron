...
and eval_stmts (p: _ prog) (n: meta node) (s: stmts) (m: metric): unit = List.iter s ~f:(fun stmt -> eval_stmt p n stmt m)
let eval (p: _ prog) (n: meta node) (m: metric): unit =  List.iter p.order ~f:(fun pass_name -> eval_stmts p n (stmts_of_processed_proc p pass_name) m)
  ...
  | (lhs, removed :: rhs) ->
    ...
    Hashtbl.iter p.bbs ~f:(fun (BasicBlock(bb_name, stmts)) -> 
      ...
      let dirty read = (
        ...
        | ReadHasPath(Prev) | ReadProp(Prev, _) -> (match removed.next with Some x -> bb_dirtied x bb_name m | None -> ())
        | ReadHasPath(Next) | ReadProp(Next, _) -> (match removed.prev with Some x -> bb_dirtied x bb_name m | None -> ())
...
let add_children (p: _ prog) (x: meta node) (y: meta node) (n: int) (m: metric): unit =
  ...
  Hashtbl.iter p.bbs ~f:(fun (BasicBlock(bb_name, stmts)) -> 
    ...
    let dirty read = (
      ...
      | ReadHasPath(First) | ReadHasPath(Last) -> if phys_equal (List.length x.children) 1 then bb_dirtied x bb_name m else ()
      | ReadProp(First, _) -> if List.is_empty lhs then bb_dirtied x bb_name m else ()
      | ReadProp(Last, _) -> if List.is_empty rhs then bb_dirtied x bb_name m else ()
      | ReadHasPath(Prev) | ReadProp(Prev, _) -> (match y.next with Some x -> bb_dirtied x bb_name m | None -> ())
      | ReadHasPath(Next) | ReadProp(Next, _) -> (match y.prev with Some x -> bb_dirtied x bb_name m | None -> ())
      | ReadProp(Self, _) -> ()
      | _ -> raise (EXN (show_read read))) in
    List.iter reads ~f:dirty)

let rec recalculate_aux (p: _ prog) (n: meta node) (down_name: string option) (up_name: string option) (m: metric) = 
  meta_read m n.id;
  ...
  if optional_recursive_dirtied down_name || optional_recursive_dirtied up_name then
    ...
    | None -> () 
    | Some dn -> rerun_if_dirty dn);
    List.iter n.children ~f:(fun n -> recalculate_aux p n down_name up_name m);
    (match up_name with
    | None -> () 
    | Some un -> rerun_if_dirty un))
  else ();
  let clean oname = 
    match oname with
    | None -> ()
    | Some name -> Hashtbl.set n.m.recursive_bb_dirtied ~key:name ~data:false in
  clean down_name; clean up_name

let recalculate (p: _ prog) (n: meta node) (m: metric): unit = 
  List.iter p.order ~f:(fun name -> let down, up = get_bb_from_proc p name in recalculate_aux p n down up m)