...
module EVALDB : Eval = struct
  ...
  let rec set_recursive_bb_dirtied (n : meta node) (bb_name : string)
      (m : metric) : unit =
    meta_write m n.id;
    if Hashtbl.find_exn n.m.recursive_bb_dirtied bb_name then ()
      ...
      match n.parent with
      | Some p -> set_recursive_bb_dirtied p bb_name m
      | None -> ())
  ...
  let rec initialize_bb_dirtied_aux (n : meta node) (bb_name : string)
      (m : metric) : unit =
    meta_write m n.id;
    Hashtbl.set n.m.bb_dirtied ~key:bb_name ~data:true;
    Hashtbl.set n.m.recursive_bb_dirtied ~key:bb_name ~data:true;
  ...
  let initialize_bb_dirtied (n : meta node) (bb_name : string) (m : metric) :
      unit =
    initialize_bb_dirtied_aux n bb_name m;
    set_recursive_bb_dirtied n bb_name m;
    m.meta_write_count <- m.meta_write_count - 1

  let prop_modified (p : _ prog) (n : meta node) (prop_name : string)
      (m : metric) : unit =
          ...
          | ReadProp (path, read_prop_name) ->
              if String.equal prop_name read_prop_name then
                List.iter (reversed_path path n) ~f:(fun dirtied_node ->
                    bb_dirtied dirtied_node bb_name m)
              else ()
          | ReadHasPath _ ->
              () (*property being changed cannot change haspath status*)
        in
  ...
  let rec eval_stmt (p : _ prog) (n : meta node) (s : stmt) (m : metric) : unit
    ...
    | Write (path, prop_name, expr) ->
        ...
        if Option.is_some (Hashtbl.find new_node.dict prop_name) then
          prop_modified p new_node prop_name m
        else ();
        ...
        Hashtbl.iter p.bbs ~f:(fun (BasicBlock (bb_name, stmts)) ->
            ...
            let dirty read =
              ...
              | ReadHasPath First | ReadHasPath Last ->
                  if List.is_empty x.children then bb_dirtied x bb_name m
                  else ()
              | ReadProp (First, _) ->
                  if List.is_empty lhs then bb_dirtied x bb_name m else ()
              | ReadProp (Last, _) ->
                  if List.is_empty rhs then bb_dirtied x bb_name m else ()
              | ReadHasPath Prev | ReadProp (Prev, _) -> (
                  match removed.next with
                  | Some x -> bb_dirtied x bb_name m
                  | None -> ())
  ...
  let add_children (p : _ prog) (x : meta node) (y : meta node) (n : int)
      (m : metric) : unit =
    ...
    Hashtbl.iter p.bbs ~f:(fun (BasicBlock (bb_name, stmts)) ->
        ...
        let dirty read =
          ...
          | ReadHasPath First | ReadHasPath Last ->
              if phys_equal (List.length x.children) 1 then
                bb_dirtied x bb_name m
              else ()
          | ReadProp (First, _) ->
              if List.is_empty lhs then bb_dirtied x bb_name m else ()
          | ReadProp (Last, _) ->
  ...
  let rec recalculate_aux (p : _ prog) (n : meta node)
      (down_name : string option) (up_name : string option) (m : metric) =
    meta_read m n.id;
    ...
    let optional_recursive_dirtied name =
      match name with
      | None -> false
      | Some name -> Hashtbl.find_exn n.m.recursive_bb_dirtied name
    in
    if
      optional_recursive_dirtied down_name || optional_recursive_dirtied up_name
    then (
    ...
    let clean oname =
      match oname with
      | None -> ()
      | Some name -> Hashtbl.set n.m.recursive_bb_dirtied ~key:name ~data:false
    in
    clean down_name;
    clean up_name
