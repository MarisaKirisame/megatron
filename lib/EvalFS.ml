...
let make_node (p: _ prog) (children: unit node list): unit node = 
  ignore p; {
  id = count();
...
let rec eval_stmt (p: _ prog) (n: unit node) (s: stmt) (m: metric): unit = 
  match s with
  | ChildrenCall(proc_name) -> List.iter n.children ~f:(fun n -> eval_stmts p n (stmts_of_processed_proc p proc_name) m)
...
and eval_stmts (p: _ prog) (n: unit node) (s: stmts) (m: metric): unit = List.iter s ~f:(fun stmt -> eval_stmt p n stmt m)

let eval (p: _ prog) (n: unit node) (m: metric) = 
