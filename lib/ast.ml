...
let show_prog (p : unit prog) : string =
  Hashtbl.fold p.procs ~init:""
    ~f:(fun ~key ~data:(ProcessedProc (_, stmts)) acc ->
      acc ^ key ^ " -> " ^ show_stmts stmts ^ "\n")
  ^ Hashtbl.fold p.bbs ~init:""
      ~f:(fun ~key ~data:(BasicBlock (_, stmts)) acc ->
        acc ^ key ^ " -> " ^ show_stmts stmts ^ "\n")
...
let prog_of_prog_def (p : prog_def) : unit prog =
    ...
    let transformed_stmts =
      List.fold_right (split false stmts []) ~init:[] ~f:(fun stmts acc ->
          ...
          | _ ->
              ...
              Hashtbl.add_exn bb ~key:bb_name
                ~data:(BasicBlock (bb_name, stmts));
              BBCall bb_name :: acc)
    in
  ...
  in
  {
    ...
    bbs = bb;
    procs =
      Hashtbl.of_alist_exn (module String) (List.map p.proc_defs ~f:transform);
    order = p.order_decls;
  }

type read = ReadProp of path * string | ReadHasPath of path [@@deriving show]

let string_of_reads (reads : read list) : string =
  List.to_string reads ~f:show_read
...
let rec reads_of_expr (e : expr) : read list =
  ...
  | Int _ -> []
  | IfExpr (x, y, z) ->
      List.append (recurse x) (List.append (recurse y) (recurse z))
